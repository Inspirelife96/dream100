...
NSString * const IAPPurchaseCancelNotification    = @"IAPPurchaseCancelNotification";
NSString * const IAPRestoreCancelNotification     = @"IAPRestoreCancelNotification";
NSString * const IAPRestoreCompleteNotification   = @"IAPRestoreCompleteNotification";
NSString * const IAPPaymentRemovedNotification    = @"IAPPaymentRemovedNotification";
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions
{
	for(SKPaymentTransaction * transaction in transactions)
	{
		switch (transaction.transactionState )
		{
            ...
            {
                self.message = [NSString stringWithFormat:@"购买%@失败。",transaction.payment.productIdentifier];
...
- (void)paymentQueue:(SKPaymentQueue *)queue restoreCompletedTransactionsFailedWithError:(NSError *)error
{
    if (error.code != SKErrorPaymentCancelled)
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:IAPRestoreCancelNotification object:self];
    }
}
...
- (void)paymentQueueRestoreCompletedTransactionsFinished:(SKPaymentQueue *)queue
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:IAPRestoreCompleteNotification object:self];
}
...
-(void)completeTransaction:(SKPaymentTransaction *)transaction forStatus:(NSInteger)status
{
    ...
    if (transaction.error.code != SKErrorPaymentCancelled)
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:IAPPurchaseCancelNotification object:self];
    }
...
-(BOOL)isPurchasedProduct:(NSString *)productIdentifier
{
    BOOL hasPurchased = NO;
    
    for (int i = 0; i < self.productsPurchased.count; i++) {
        SKPaymentTransaction *paymentTransaction = self.productsPurchased[i];
        if ([paymentTransaction.payment.productIdentifier isEqualToString:productIdentifier]) {
            hasPurchased = YES;
        }
    }
    
    for (int i = 0; i < self.productsRestored.count; i++) {
        SKPaymentTransaction *paymentTransaction = self.productsRestored[i];
        if ([paymentTransaction.payment.productIdentifier isEqualToString:productIdentifier]) {
            hasPurchased = YES;
        }
    }
