...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    if ((response.invalidProductIdentifiers).count > 0)
    {
        ...
        //[self.productRequestResponse addObject:model];
        self.invalidProductIds = [NSMutableArray arrayWithArray:response.products];
    }
...
// Return the product's price matching a given product identifier
-(NSString *)priceMatchingProductIdentifier:(NSString *)identifier
{
    ...
    for (SKProduct *product in self.availableProducts)
    {
        if ([product.productIdentifier isEqualToString:identifier])
        {
            ...
            productPrice = [numberFormatter stringFromNumber:product.price];
        }
    }
    return productPrice;
}
